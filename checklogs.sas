...
/* 수정일	| 수정 내용							*/
/*----------------------------------------------*/
/* 250807 	| 결과표 형식 수정 (개수, 줄 번호)		*/
/* 250805 	| dir 출력값 형식 변경				*/
/* 250804 	| checklogs.sas 생성					*/
/************************************************/
...
%macro checklogs(loc=, /* location of where the log files are stored 		*/
	...
	data logs (keep = flog fdat ftim filename numtok);
		...
		%if &sysscp = WIN %then %do;
			do j = 5 to numtok;
				tlog = tranwrd(scan(filename, j, " "), ".log", "");			*250805 Modified: '4 - j' -> 'j' (Because of the dir command output from Korean locale system);
				flog = catx(" ", tlog, flog);
			end;
			fdat = put(input(scan(filename, 1, " "), yymmdd10.), date9.);
	...
	%do %while ("&lg" ne "");
		...
		data logck&x;
			...
			input line $1000.;
			line_num = _N_; *250807 Modified: To check the which line has undesirable message;
			/* keep only the records that had an undesirable message */
			if 	index(upcase(line), "WARNING") or
				index(upcase(line), "ERROR") or 											/* 250807 Modified: "ERROR:" -> "ERROR" */
		...
		%if &numobs = 0 %then %do;
			data logck&x._2;
				...
				logdt = "&dt";
				logtm = "&tm";
				line_num = 0; *250807 Modified: To check the which line has undesirable message;
	...
	proc sort data=alllogs presorted;
		by line_num lognm; *250807 Modified: Sort by line number first;
	run;

	/* 250807 Modified: To check the total number of undesirable message */
	data alllogs_final;
		set alllogs;
		report_line_num + 1;
	run;
	...
	proc report data=alllogs_final ls=140 ps=43 spacing=1 missing nowindows headline;
		column lognm logdt logtm report_line_num line_num line logrs;
		define lognm / order style(column)=[width=12%] "Log Name";
		define logdt / display style(column)=[width=8%] "Log Date";
		define logtm / display style(column)=[width=8%] "Log Time";
		define report_line_num / display style(column)=[width=5%] "Index"; 					*250807 Modified: Add columns ("Index", "Line Number") to report;
		define line_num / display style(column)=[width=7%] "Line Number";
